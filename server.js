...
// ─── POST /api/create-link-token-auth ─────────────────────
// Auth-only link token (no transfer) — used to connect an
// exchange account and view the portfolio without paying.
app.post("/api/create-link-token-auth", async (req, res) => {
  try {
    const body = {
      userId: `user_${uuidv4().slice(0, 8)}`,
    };

    console.log("\n─── Creating auth-only link token ───");
    ...
    console.log("Auth-only link token created ✓");

    res.json({
      linkToken: data.content?.linkToken || data.linkToken,
    });
  } catch (err) {
    console.error("Auth link token error:", err);
    res.status(500).json({ error: "Internal server error" });
  }
});

// ─── POST /api/portfolio ──────────────────────────────────
// Called by the frontend after Mesh Link returns an accessToken.
// Fetches the user's holdings from their connected account
// via Mesh's portfolio API.
app.post("/api/portfolio", async (req, res) => {
  try {
    const { authToken, brokerType } = req.body;

    if (!authToken || !brokerType) {
      return res.status(400).json({ error: "authToken and brokerType are required" });
    }
    ...
    if (!response.ok) {
      console.error("Mesh portfolio API error:", data);
      return res.status(response.status).json({
    ...
    console.log(`  Crypto positions: ${cryptoPositions.length}`);
    console.log(`  Equity positions: ${equityPositions.length}`);

    res.json({
      status: content.status,
      institutionName: content.institutionName || brokerType,
      accountName: content.accountName || "",
      accountId: content.accountId || "",
      cryptocurrencyPositions: cryptoPositions,
